...
public class Displays {
	...
	/**
     * Displays a delivery that was already made object elements as follows:
     * Delivery ID, desired Delivery time.
     *
     * @param delivery
     */
    public void displayDeliveryAlreadyMade(Delivery delivery) {
        String strId = delivery.getDeliveryID();
        strId = strId.substring(1, strId.length());
        Integer id = Integer.parseInt(strId);

        System.out.println(String.format("D%04d - %s", id, delivery.getEstimatedDeliveryDuration()));
    }
	...
	public void displayDeliveriesAlreadyMadeByDriver(Driver driver) {
        DeliveryList deliveryList = driver.getDeliveriesMadeList();

        boolean done = false;
        Delivery currentDelivery;

        deliveryList.goToFrontOfList();

        while (!done) {
            currentDelivery = deliveryList.getNextDelivery();

            if( currentDelivery == null) {
                done = true;
            }
            else {
                displayDeliveryAlreadyMade(currentDelivery);
            }
        }
    }

	public void DeliveriesUnassigned(DeliveryList deliveryList) {
		// TODO Auto-generated method stub
			...
			System.out.println("\nDeliveries currently unassigned: ");
			System.out
					.println("===========================================================");
			delivery = deliveryList.getNextDelivery();
			while(delivery != null){
