		...
		if (registrationNumber.compareTo((Integer)notificationNumber) == 0) {
			...
			if (myDriver != null) {
				...
				if (myDelivery != null)	{	
					...
					{
						if(((this.getDriverSchedule().validateNotOvertime(myDriver, myDelivery) && this.getDriverSchedule().verifyNoConflict(myDriver, myDelivery)) == true)){
							isOkay = true;
						}
						else{
							isOkay = false;
							System.out.println("\n\n *** Can't assign a delivery, driver is in conflict. ***");
						}
					}
					else
					{
						isOkay = false;
						System.out.println("\n\n *** Can't assign a delivery already assigned. ***");
					}
					
					//if driver and delivery is valid
